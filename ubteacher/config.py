...
def add_ubteacher_config(cfg):
    ...
    _C.MOCO.UNLABELED_CONTRASTIVE_IOU_THRES = 0.8
    ...
    _C.MOCO.WARMUP_EMA = False
    _C.MOCO.INSERT_NOISE_TO_FEATURES = False
    _C.MOCO.RESUME_AFTER_BURNUP = False
    _C.MOCO.SPLIT_DATA = False

    _C.MOCO.ENABLED_UNLABELED_CONTRASTIVE_LOSS = True
    _C.MOCO.ENABLED_UNLABELED_QUEUE_UPDATE = True
    _C.MOCO.ENABLED_LABELED_CONTRASTIVE_LOSS = True
    _C.MOCO.ENABLED_LABELED_QUEUE_UPDATE = True

    _C.MOCO.CLASSWISE_QUEUE = False
    _C.MOCO.CLASSWISE_QUEUE_ITEMS_PER_CLASS = 300
    _C.MOCO.CLASSWISE_QUEUE_WARMUP = 7000 # include burnup stage
    # RoI heads regression uncertainty
    _C.MODEL.ROI_HEADS.UNCERTAINTY = False
    _C.MODEL.ROI_HEADS.BBOX_CLS_LOSS_WEIGHT = 1.0
    _C.MODEL.ROI_HEADS.BBOX_REG_UNCERTAINTY_LOSS_WEIGHT = 0.1
    _C.MODEL.ROI_HEADS.BBOX_REG_UNCERTAINTY_LOSS_REGULARIZATION_WEIGHT = 1.0
