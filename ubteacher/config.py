...
def add_ubteacher_config(cfg):
    ...
    _C.SEMISUPNET.UNSUP_REG_LOSS_WEIGHT = 4.0
    ...
    _C.MOCO.CONT_IOU_THRESHOLD = 0.8
    _C.MOCO.CONT_CLASS_THRESHOLD = 0.8
    _C.MOCO.CONT_SCORE_THRESHOLD = 0.7

    _C.MOCO.CONTRASTIVE_LOSS_WEIGHT_DECAY = False
    _C.MOCO.CONTRASTIVE_LOSS_WEIGHT_DECAY_STEP = 50000
    _C.MOCO.BOX_JITTERING = False
    _C.MOCO.JITTER_FRAC = 0.0
    _C.MOCO.JITTER_TIMES = 1
    _C.MOCO.CONT_PREDICTION_HEAD = True
    _C.MOCO.CLASS_AWARE_CONTRASTIVE = True
    _C.MOCO.CONTRASTIVE_LEARNING = True

    _C.MOCO.TARGET_GENERATE_MODEL = "teacher" # "student"

    _C.MOCO.CONTRASTIVE_LOSS_TYPE = 'infoNCE' # 'byol'

    # RoI heads regression uncertainty
    _C.MODEL.ROI_HEADS.BBOX_CLS_LOSS_WEIGHT = 1.0
    _C.MODEL.ROI_HEADS.BBOX_REG_UNCERTAINTY_LOSS_WEIGHT = 1.0
    _C.MODEL.ROI_HEADS.BBOX_REG_UNCERTAINTY_LOSS_REGULARIZATION_WEIGHT = 0.25
    _C.MODEL.ROI_HEADS.UNCERTAINTY_START_ITER = 0
    
    _C.MODEL.ROI_BOX_HEAD.BOX_ENCODE_TYPE = "xywh" # "xywh", "xyxy"

    _C.UNCERTAINTY = CN()
    _C.UNCERTAINTY.UNCERTAINTY_THRESHOLD = 0.0 
    _C.UNCERTAINTY.PSEUDO_LABEL_REG = False
    _C.UNCERTAINTY.UNLABEL_REG_LOSS_TYPE = 'none'

    # soft teacher like uncertainty
    _C.SOFT_TEACHER = CN()
    _C.SOFT_TEACHER.UNCERTAINTY_THRESHOLD = 0.02
    _C.SOFT_TEACHER.JITTERING_TIMES = 10
    _C.SOFT_TEACHER.JITTERING_FRAC = 0.06
    
    # 3DIoUMatch like uncertainty 
    _C.IOUNET = CN()
    _C.IOUNET.TRAINING_WITH_JITTERING = False
    _C.IOUNET.JITTERING_TIMES = 50
    _C.IOUNET.JITTERING_FRAC = 0.3
    _C.IOUNET.IOU_THRESHOLD = 0.5
    _C.IOUNET.IOULOSS_WEIGHT = 1.0
