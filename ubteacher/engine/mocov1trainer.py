...
class MoCov1Trainer(DefaultTrainer):
    def __init__(self, cfg):
        ...
        TODO:
        1. contrastive branch까지 있는 모델을 생성 
        2. unlabeled data(weak aug)로 pseudo label을 만들고
        model을 labeled data(weak, strong) + unlabeled(strong) 으로 학습
        3. contrastive loss는 어떻게 추가해야하지? 
        ...
        # create an teacher model(Only for roi head)
        roi_head_teacher = build_roi_heads(cfg, model.backbone.output_shape()),
        self.roi_head_teacher = roi_head_teacher
        ...
        self.mocov1_ts_model = MoCov1TSModel(roi_head_teacher, model)

        self.checkpointer = DetectionTSCheckpointer(
            self.mocov1_ts_model,
            cfg.OUTPUT_DIR,
            optimizer=optimizer,
    ...
    def run_step_full_semisup(self):
        ...
        if self.iter < self.cfg.SEMISUPNET.BURN_UP_STEP:
            ...
            record_dict = {}
            #  generate the pseudo-label using teacher model(RoI heads)
            ...
            with torch.no_grad():
                ...
                _, proposals_rpn_k, _, _, features_k = self.model(data_k, branch="generate_rpn_proposals")

                proposals_roih_k, contrastive_features_k, box_features_k = self.roi_head_teacher(
                    images=None,
                    features=features_k, 
                    proposals=proposals_rpn_k, 
                    branch="forward"
                )
            #  Pseudo-labeling
            ...
            all_unlabel_data = unlabel_data_q # unlabeled의 경우는 strong aug로만 학습
    ...
    def add_feature_noise(self, feature_maps, dropblock_size, spatial_dropout_ratio):
        m = nn.Dropout2d(p=1/64)
        # input = torch.randn(20, 16, 32, 32)
        # output = m(input)

        # (bsize, n_feats, height, width)
        # x = torch.rand(100, 10, 16, 16)
        # drop_block = DropBlock2D(block_size=2, drop_prob=1/64)
        # regularized_x = drop_block(x)

        return noisy_box_features
